...
public final class Record {
    ...
    public Record(long seq, KeyValuePair[] pairs, Signer signer) {
        ...
        try {
            this.rlp = RLPDecoder.RLP_STRICT.wrapList(record);
    ...
    public static Record parse(String enrString) throws DecodeException {
        if(enrString.startsWith(ENR_PREFIX)) {
            return decode(Strings.decode(enrString.substring(ENR_PREFIX.length()), Strings.BASE_64_URL_SAFE));
        }
    ...
    public long getSeq() throws DecodeException {
        RLPListIterator iter = new RLPListIterator(getRLP(), RLPDecoder.RLP_STRICT);
        iter.next();
    ...
    public String toString() {
        return ENR_PREFIX + rlp.toString(Strings.BASE_64_URL_SAFE);
    }
}
