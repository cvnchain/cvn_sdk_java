...
final class TypeFactory {
    ...
    private static final ABIType<byte[]> NAMELESS_BYTES_TYPE = new ArrayType<>("bytes", ArrayType.BYTE_ARRAY_CLASS, true, ByteType.UNSIGNED, ArrayType.DYNAMIC_LENGTH, ArrayType.BYTE_ARRAY_ARRAY_CLASS_NAME);

    private static final ABIType<String> NAMELESS_STRING_TYPE = new ArrayType<>("string", ArrayType.STRING_CLASS, true, ByteType.UNSIGNED, ArrayType.DYNAMIC_LENGTH, ArrayType.STRING_ARRAY_CLASS_NAME);

    private static final ABIType<BigDecimal> NAMELESS_FIXED_TYPE = new BigDecimalType("fixed128x18", BaseTypeInfo.FIXED_BIT_LEN, BaseTypeInfo.FIXED_SCALE, false);
    private static final ABIType<BigDecimal> NAMELESS_UFIXED_TYPE = new BigDecimalType("ufixed128x18", BaseTypeInfo.FIXED_BIT_LEN, BaseTypeInfo.FIXED_SCALE, true);
    private static final ABIType<BigDecimal> NAMELESS_DECIMAL_TYPE = new BigDecimalType("decimal", BaseTypeInfo.DECIMAL_BIT_LEN, BaseTypeInfo.DECIMAL_SCALE, false);
        ...
        try {
            ...
            if (type.charAt(idxOfLast) == ']') { // array
                ...
                if (arrayOpenIndex == fromIndex) { // i.e. []
                    length = ArrayType.DYNAMIC_LENGTH;
                } else { // e.g. [4]
                    try {
    ...
    private static ABIType<?> resolveBaseType(String baseTypeStr, boolean isElement, boolean nameless) throws ParseException, ClassNotFoundException {
        ...
        if(info != null) {
            ...
            case "bytes":   return nameless ? NAMELESS_BYTES_TYPE : new ArrayType<>(baseTypeStr, ArrayType.BYTE_ARRAY_CLASS, true, ByteType.UNSIGNED, ArrayType.DYNAMIC_LENGTH, ArrayType.BYTE_ARRAY_ARRAY_CLASS_NAME);
            case "string":  return nameless ? NAMELESS_STRING_TYPE : new ArrayType<>(baseTypeStr, ArrayType.STRING_CLASS, true, ByteType.UNSIGNED, ArrayType.DYNAMIC_LENGTH, ArrayType.STRING_ARRAY_CLASS_NAME);
            case "decimal": return nameless ? NAMELESS_DECIMAL_TYPE : new BigDecimalType(baseTypeStr, BaseTypeInfo.DECIMAL_BIT_LEN, BaseTypeInfo.DECIMAL_SCALE, false);
            case "fixed":
            case "fixed128x18":
                            return nameless ? NAMELESS_FIXED_TYPE : new BigDecimalType("fixed128x18", BaseTypeInfo.FIXED_BIT_LEN, BaseTypeInfo.FIXED_SCALE, false);
            case "ufixed":
            case "ufixed128x18":
                            return nameless ? NAMELESS_UFIXED_TYPE : new BigDecimalType("ufixed128x18", BaseTypeInfo.FIXED_BIT_LEN, BaseTypeInfo.FIXED_SCALE, true);
            default:        return null;
            }
        }
